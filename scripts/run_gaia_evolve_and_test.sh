...
#   3. freeze             → freeze latest team/vNN under a new agents/ pool name
...
case "$MODE" in
  ...
  evolve)
    ...
    echo "  bash scripts/freeze_evolved_team.sh ${RUN_ID}_evolve latest pool_GAIA_evolved_final"
    ;;
  ...
  test)
    ...
    if [[ -z "$POOL" ]]; then
        ...
        echo "Example: $0 test pool_GAIA_evolved_final --rollout 3"
        exit 1
    fi
