...
def load_project_env() -> None:
    """Load the explicitly selected experiment settings.

    Match ``core.llm``: an explicit ``META_TEAM_ENV_FILE`` is authoritative;
    only fall back to the project ``.env`` when no file was selected.
    """
    configured = os.environ.get("META_TEAM_ENV_FILE", "").strip()
    env_path = Path(configured).expanduser() if configured else ROOT / ".env"
