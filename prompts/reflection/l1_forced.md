...
- "A teammate took too long on a subtask" → do not record it in L1; stable
  capability observations are Chairman-only L2 memory, while reusable
...
- **Do NOT write observations about teammates or handoff procedures.**
  Chairman-only L2 owns stable capability observations; the handoff reflector
  owns reusable family-scoped coordination rules.
