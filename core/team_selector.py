...
中文说明：Selector 只选择 family 及候选成员，不直接执行任务或启动 Agent。
"""
...
class TeamSelector(Protocol):
    async def select(self, context: TeamSelectionContext) -> TeamSelection:
        # 中文：先尝试复用已有 family；选择失败时进入可审计的冷启动回退。
        # Selector 只决定 family 和候选成员范围，不在此启动 Agent；
        # 实际招募和分工由后续 Runner/Chairman 执行。
...
class LLMTeamSelector:
    ...
    # 中文：模型提出选择建议，代码负责校验 ID、成员范围和冷启动回退。
    ...
    async def select(self, context: TeamSelectionContext) -> TeamSelection:
        ...
        if action == "reuse":
            ...
            # 冷启动先分配临时 family 标识并选择候选成员；可复用 family
            # 的抽象描述由后续 reflector 负责，不在 ID 中编码任务主题。
            family_payload = {
                **family_payload,
