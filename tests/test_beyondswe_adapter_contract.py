...
def test_execution_policy_matches_actual_beyondswe_agents_and_tools():
    ...
    assert policy.role_instruction_aliases["context_analyst"] == (
        "repo_analyst",
    )
    assert policy.role_instruction_aliases["implementer"] == ("developer",)
    assert policy.role_instruction_aliases["verifier"] == (
        "reviewer", "test_engineer", "integration_verifier",
    )
    assert policy.role_instruction_aliases["integrator"] == (
        "integration_verifier",
    )


def test_depmigrate_shared_roles_receive_dependency_specialist_rules():
    policy = BeyondSWEAdapter().execution_policy({"_task_type": "DepMigrate"})

    for shared_role in ("context_analyst", "implementer", "verifier", "integrator"):
        aliases = policy.role_instruction_aliases[shared_role]
        assert "dependency_specialist" in aliases
