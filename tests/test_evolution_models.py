...
def test_new_handoff_decision_requires_semantic_rule_id() -> None:
    base = dict(
        action="create",
        handoff_family_id="hf_web_research",
        from_agent="web_agent",
        to_agent="file_agent",
        reason="Reusable evidence transfer",
        instruction="Send cited evidence.",
        trigger_description="Use when source evidence needs verification.",
    )
    HandoffDecision(rule_id="history_source_disambiguation", **base).validate()
    with pytest.raises(ValueError, match="random hex"):
        HandoffDecision(rule_id="hr_a81f03d9c2e1", **base).validate()
