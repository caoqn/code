...
中文说明：原语工具是 Agent 与 Runner 的受控接口，负责招募、等待、提交答案和
终止任务；权限检查在工具处理器内部完成。
"""


def build_primitive_tools(runner, agent_name: str | None = None) -> list[dict]:
    """Build the core team tool schemas (recruit, send_message, terminate, etc.)."""
    tools = _build_schemas(runner, agent_name=agent_name)
...
def rebuild_primitive_schemas(
    runner,
    include_chairman_tools: bool = True,
    agent_name: str | None = None,
) -> list[dict]:
    return _build_schemas(
        runner,
        include_chairman_tools=include_chairman_tools,
...
def _build_schemas(
    ...
    tools.append({
        ...
        "function": {
            "name": "list_pool",
            "description": (
                "List agents available in the selected family with their capabilities. "
    ...
    if runner.enable_reflection and runner._phase != "task_execution":
        tools.extend(_build_reflection_schemas(
            runner, phase=runner._phase, agent_name=agent_name,
        ))

    return tools


def _build_reflection_schemas(
    runner,
    phase: str = "",
    agent_name: str | None = None,
) -> list[dict]:
    ...
    if phase == "l1_reflection":
        ...
        l2_agents = set(runner.l2_reflection_agents())
        if agent_name is not None and agent_name not in l2_agents:
            return tools
        all_agent_names = list(runner.agents.keys())
        if (
            agent_name == runner.chairman_name
            and not runner.member_teammate_profiles_enabled
        ):
            all_agent_names = runner.chairman_profile_targets()
        else:
            pool_agents = runner.list_pool_agents()
...
def _build_reflection_handlers(runner) -> dict:
    ...
    def _check_phase(tool_name: str, caller_name: str) -> str | None:
        ...
        if phase == "l2_reflection":
            if tool_name in _L2_TOOLS:
                if caller_name not in runner.l2_reflection_agents():
                    return (
                        "Error: you are not an L2 profile-reflection agent for "
                        "this run. Execution members do not create teammate profiles."
                    )
                return None
    ...
    def _check_already_completed(tool_name: str, caller_name: str) -> str | None:
        ...
        if phase == "l2_reflection" and caller_name in runner._l2_completed:
            if tool_name in _L2_TOOLS:
                return "You have already completed L2 reflection; the session is ending."
        return None
    ...
    async def _update_teammate_profile(
        ...
        if not agent_dir:
            return f"Error: no agent directory found for {_caller_name}."
        if (
            _caller_name == runner.chairman_name
            and not runner.member_teammate_profiles_enabled
            and teammate_name not in runner.chairman_profile_targets()
        ):
            return (
                "Error: Chairman profiles may only target agents actually "
                "recruited in this task."
            )
    ...
    async def _skip_l2(reason: str = "", _caller_name: str = "", **kwargs) -> str:
        ...
        return result + progress + (
            ...
            "The session ends once all agents have completed L2."
        )
